...
void App::setup() {
    uint32_t tiles_x       = (cfg_.img_width  + cfg_.tile_width  - 1u) / cfg_.tile_width;
    uint32_t tiles_y       = (cfg_.img_height + cfg_.tile_height - 1u) / cfg_.tile_height;
    uint32_t tiles_per_frame = tiles_x * tiles_y;

    assembler_ = std::make_shared<common::assembler::FrameAssemblerProcObj>(
        cfg_.img_width, cfg_.img_height, tiles_per_frame);
...
float App::get_fps() const {
    return controller_->get_fps();
}
...
void App::set_target_fps(float fps)      { controller_->set_target_fps(fps); }
void App::set_julia_c(float r, float i)  { controller_->set_julia_c(r, i); }
void App::set_zoom(float zoom)           { controller_->set_zoom(zoom); }
void App::set_center(float cx, float cy) { controller_->set_center(cx, cy); }
void App::set_max_iter(uint32_t n)       { controller_->set_max_iter(n); }
